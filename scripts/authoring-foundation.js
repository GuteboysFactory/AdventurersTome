...
const ATAF_WORLD_CATEGORIES = Object.freeze({ npc: "NPCs", location: "Locations", faction: "Factions", item: "Items", lore: "Lore" });

const atAfPending = new Map();
let atAfQueued = false;
let atAfBadgeTimer = null;
...
function atAfCanCreate() {
  if (game.user?.isGM) return true;
  try { return Boolean(game.user?.can?.("JOURNAL_CREATE") || game.user?.hasPermission?.("JOURNAL_CREATE")); } catch (_err) { return false; }
}
...
function atAfState(state, text = "") {
  ...
  window.clearTimeout(atAfBadgeTimer);
  if (state === "saved") atAfBadgeTimer = window.setTimeout(() => badge.classList.remove("is-visible"), 1500);
}

function atAfSchedule(key, save) {
  const previous = atAfPending.get(key);
  if (previous?.timer) window.clearTimeout(previous.timer);
  const record = { save, timer: null, running: false };
  record.timer = window.setTimeout(() => void atAfFlush(key), ATAF_SAVE_DELAY);
  atAfPending.set(key, record);
  atAfState("editing");
}

async function atAfFlush(key) {
  ...
  try {
    ...
    record.timer = window.setTimeout(() => void atAfFlush(key), 1800);
    atAfPending.set(key, record);
    atAfState("error", "Save failed — retrying");
  } finally {
    window.setTimeout(() => atAfSetBulk(false), 180);
  }
}
...
function atAfPrimaryPage(journal, section) {
  ...
  if (section === "world") {
    ...
    const byId = pages.find((page) => page.id === preferred && String(page.type || "text") === "text");
    if (byId) return byId;
  }
...
function atAfWriteSummaryHtml(html, summary) {
  const host = document.createElement("div");
  host.innerHTML = String(html || "");
  let node = host.querySelector(`[${ATAF_SUMMARY_ATTR}]`);
  if (!node) {
    node = document.createElement("p");
    node.setAttribute(ATAF_SUMMARY_ATTR, "true");
    host.prepend(node);
  }
  node.textContent = String(summary || "").trim();
  return host.innerHTML;
}

async function atAfSaveSummary(journal, section, summary) {
  const page = await atAfEnsurePrimaryPage(journal, section);
  if (!page) throw new Error("No writable Journal page available");
  await page.update({ "text.content": atAfWriteSummaryHtml(page?.text?.content ?? "", summary), "text.format": atAfHtmlFormat() });
  if (section === "world") {
    const profile = foundry.utils.deepClone(journal.getFlag?.(ATAF_MODULE_ID, "worldProfile") || {});
...
function atAfMark(node, detail, kind, page = null, rich = false) {
  if (!node || !atAfCanEdit(detail.journal)) return;
  node.dataset.atAfEditable = "true";
  node.dataset.atAfJournalId = detail.journal.id;
  node.dataset.atAfSection = detail.section;
  node.dataset.atAfKind = kind;
  node.dataset.atAfMode = rich ? "rich" : "plain";
  if (page?.id) node.dataset.atAfPageId = page.id;
  node.classList.add("at-authoring-editable");
  node.title = "Click to edit — autosaves";
}
...
function atAfEnhanceInline(detail) {
  ...
  if (section === "world") {
    ...
    atAfMark(intro?.querySelector("h1"), detail, "title");
    atAfMark(intro ? [...intro.children].find((child) => child.tagName === "P") : null, detail, "summary");
    ...
    if (primary) atAfMark(knownText, detail, "page", primary, true);
    for (const surface of container.querySelectorAll(".at-world-journal-page[data-page-id] .at-world-journal-text")) {
      const page = journal.pages?.get(String(surface.closest("[data-page-id]")?.dataset?.pageId || ""));
      if (page) atAfMark(surface, detail, "page", page, true);
    }
  } else if (section === "quests") {
    const hero = container.querySelector(".at-quest-detail-hero");
    atAfMark(hero?.querySelector("h1"), detail, "title");
    atAfMark(hero?.querySelector(":scope > div > p") || hero?.querySelector("p"), detail, "summary");
  } else if (section === "sessions") {
    atAfMark(container.querySelector(".at-session-detail-head h2"), detail, "title");
    atAfMark(container.querySelector(".at-session-detail-summary"), detail, "summary");
  }
}

async function atAfEnrich(page) {
...
async function atAfRenderPages(detail) {
  ...
  for (const page of pages) {
    ...
    let body = '<p class="at-empty">Open Source in Foundry to edit this page type.</p>';
    if (type === "text") body = `<div class="at-af-page-text at-tome-richtext at-shareable-text">${await atAfEnrich(page) || '<p class="at-empty">This page is empty.</p>'}</div>`;
    else if (type === "image" && page.src) body = `<figure class="at-af-media"><img src="${atAfEscape(page.src)}" alt="${atAfEscape(page.name)}"></figure>`;
    else if (type === "video" && page.src) body = `<div class="at-af-media"><video src="${atAfEscape(page.src)}" controls playsinline></video></div>`;
    else if (type === "pdf" && page.src) body = `<div class="at-af-media at-af-pdf"><iframe src="${atAfEscape(page.src)}" title="${atAfEscape(page.name)}"></iframe></div>`;
    nav.push(`<button type="button" data-at-af-scroll-page="${atAfEscape(page.id)}"><i class="fa-solid fa-file-lines"></i><span>${atAfEscape(page.name || "Untitled")}</span></button>`);
    articles.push(`<article class="at-af-page" data-page-id="${atAfEscape(page.id)}" data-page-type="${atAfEscape(type)}"><header><span>${atAfEscape(type)}</span><h3>${atAfEscape(page.name || "Untitled page")}</h3></header>${body}</article>`);
  }
  const add = editable ? `<button type="button" class="at-secondary" data-at-af-add-page="${atAfEscape(journal.id)}"><i class="fa-solid fa-plus"></i> Page</button>` : "";
  const body = pages.length ? `<div class="at-af-pages-layout"><nav>${nav.join("")}</nav><div class="at-af-pages-document">${articles.join("")}</div></div>` : '<p class="at-empty">No readable Journal pages yet.</p>';
  shell.innerHTML = `<div class="at-af-pages-head"><div><span class="at-kicker">Journal-backed content</span><h2>${section === "quests" ? "Quest Pages" : "Session Pages"}</h2><p>Real Foundry Journal pages, authored inside Tome.</p></div>${add}</div>${body}`;
  ...
  if (editable) {
    for (const article of shell.querySelectorAll(".at-af-page[data-page-id]")) {
      ...
      if (!page) continue;
      atAfMark(article.querySelector("h3"), detail, "pageTitle", page, false);
      if (String(page.type || "text") === "text") atAfMark(article.querySelector(".at-af-page-text"), detail, "page", page, true);
    }
  }
}

function atAfKey(node) {
  return [node.dataset.atAfJournalId, node.dataset.atAfKind, node.dataset.atAfPageId || ""].join(":");
}

function atAfBegin(node, event) {
...
function atAfScheduleNode(node) {
  const journal = game.journal?.get(String(node.dataset.atAfJournalId || ""));
  if (!journal) return;
  ...
  atAfSchedule(atAfKey(node), async () => {
...
async function atAfEnsureFolder(name, parent = null) {
  ...
  const found = [...(game.folders?.contents ?? [])].find((folder) => folder.type === "JournalEntry" && folder.name === name && atAfParentId(folder) === parentId);
  return found || Folder.create({ name, type: "JournalEntry", folder: parentId });
}

async function atAfSectionFolder(section) {
  const root = await atAfEnsureFolder("Adventurer's Tome");
  return atAfEnsureFolder(section === "sessions" ? "Sessions" : section === "quests" ? "Quests" : "World", root);
}

function atAfDescendants(root) {
  const result = [];
  const walk = (parent, depth = 0) => {
    const children = [...(game.folders?.contents ?? [])].filter((folder) => folder.type === "JournalEntry" && atAfParentId(folder) === parent.id).sort((a, b) => String(a.name).localeCompare(String(b.name)));
...
function atAfModal(title, body, onSubmit) {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try { await onSubmit(new FormData(form)); overlay.remove(); }
    catch (error) { console.error("Adventurer's Tome | Create failed", error); ui.notifications.error(`Adventurer's Tome: ${error?.message || "Could not create content."}`); submit.disabled = false; }
  });
}

async function atAfOpenCreate(section) {
  const sectionFolder = await atAfSectionFolder(section);
  const options = [`<option value="">${atAfEscape(sectionFolder.name)} (root)</option>`, ...atAfDescendants(sectionFolder).map(({ folder, depth }) => `<option value="${atAfEscape(folder.id)}">${"— ".repeat(depth + 1)}${atAfEscape(folder.name)}</option>`)].join("");
  const world = section === "world" ? `<label><span>World type</span><select name="category">${Object.entries(ATAF_WORLD_CATEGORIES).map(([id, label]) => `<option value="${id}">${atAfEscape(label.replace(/s$/, ""))}</option>`).join("")}</select></label>` : "";
  const quest = section === "quests" ? '<label><span>Quest status</span><select name="status"><option value="active">Active</option><option value="dormant">Dormant</option><option value="completed">Completed</option><option value="failed">Failed</option></select></label>' : "";
  const session = section === "sessions" ? '<label><span>Session number <small>optional</small></span><input name="sessionNumber" type="number" min="1" step="1"></label>' : "";
  const body = `<label><span>Create</span><select name="kind"><option value="entry">New ${section === "world" ? "World entry" : section === "quests" ? "Quest" : "Session"}</option><option value="folder">New folder / subfolder</option></select></label><label><span>Name</span><input name="name" required autocomplete="off"></label><label><span>Parent folder</span><select name="parent">${options}</select></label>${world}${quest}${session}<p class="at-authoring-help"><i class="fa-solid fa-database"></i> Creates real Foundry Journal data.</p>`;
...
async function atAfOpenAddPage(journalId) {
  ...
  if (!journal || !atAfCanEdit(journal)) return;
  const body = '<label><span>Page title</span><input name="name" required autocomplete="off"></label><label><span>Page type</span><select name="type"><option value="text">Text</option><option value="image">Image</option><option value="video">Video</option><option value="pdf">PDF</option></select></label><label><span>Media source <small>for Image / Video / PDF</small></span><input name="src" autocomplete="off"></label>';
  atAfModal(`Add page to ${journal.name}`, body, async (data) => {
    ...
    const sort = Math.max(0, ...[...(journal.pages?.contents ?? [])].map((page) => Number(page.sort || 0))) + 100000;
    const pageData = { name, type, sort };
...
function atAfAddPageButton(detail) {
  ...
  button.dataset.atAfAddPage = detail.journal.id;
...
async function atAfMigrateWorldSummaries() {
  ...
  if (migrated) console.info(`Adventurer's Tome | Migrated ${migrated} World summaries into Journal-backed content.`);
}
...
Hooks.once("ready", () => {
  ...
  document.addEventListener("click", (event) => {
    ...
    const add = event.target.closest("[data-at-af-add-page]");
    if (add) { event.preventDefault(); event.stopImmediatePropagation(); void atAfOpenAddPage(String(add.dataset.atAfAddPage || "")); return; }
    const scroll = event.target.closest("[data-at-af-scroll-page]");
    if (scroll) {
      event.preventDefault();
      scroll.closest(".at-authoring-pages")?.querySelector(`.at-af-page[data-page-id="${CSS.escape(String(scroll.dataset.atAfScrollPage || ""))}"]`)?.scrollIntoView?.({ behavior: "smooth", block: "start" });
      return;
    }
    ...
    atAfBegin(editable, event);
  }, true);
  document.addEventListener("input", (event) => {
    const node = event.target.closest?.("[data-at-af-editing='true']");
    if (node) atAfScheduleNode(node);
  }, true);
  document.addEventListener("focusout", (event) => {
    const node = event.target.closest?.("[data-at-af-editing='true']");
    if (node) void atAfFinish(node);
  }, true);
  new MutationObserver(atAfQueue).observe(document.body, { childList: true, subtree: true });
  atAfQueue();
});
